...
export async function GET(req: NextRequest) {
  ...
  const cacheKey = `products:${search}:${inStockOnly}:${offset}:${limit}:${categoryId || ''}:${sort}`;
  const cached = cacheGet<{ products: unknown[]; total: number }>(cacheKey);
  if (cached) return NextResponse.json({ ...cached, fromCache: true });

  try {
    const result = await getProducts(session.user.uid, session.user.password, { search, inStockOnly, offset, limit, categoryId, sort });

    // Pricelist pricing: Mobile API already returns the correct customer price
    // via sign_in authentication — no additional overlay needed
